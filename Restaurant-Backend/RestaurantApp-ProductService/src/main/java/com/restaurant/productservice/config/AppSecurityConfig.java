package com.restaurant.productservice.config;
...
import com.restaurant.productservice.filter.JWTTokenValidatorFilter;
...
//		        config.setAllowedOrigins(Collections.singletonList(CorsConfiguration.ALL));
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		...
		http.sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS).and().cors()
				...
				.csrf((csrf) -> csrf.ignoringRequestMatchers("/api/v1/productservice/**"))
				.addFilterBefore(new JWTTokenValidatorFilter(), BasicAuthenticationFilter.class).exceptionHandling().and()
				.authorizeHttpRequests((request)->request
						.requestMatchers("/api/v1/productservice/verify/**").hasAnyRole("ADMIN","CUSTOMER","SOCIAL_USER")
				.requestMatchers("/api/v1/productservice/product/**","/api/v1/productservice/user/**").permitAll());
		return http.build();
	}
